...
<div class="container">
    ...
    <div class="card mb-4">
        <div class="card-header bg-success text-white d-flex justify-content-between align-items-center">
            ...
            <button id="btnAddTransaction" class="btn btn-light btn-sm">
                ...
                <tbody>
                    ...
                    <?php if (!empty($riwayat_transaksi)): ?>
                        <?php foreach ($riwayat_transaksi as $transaksi): ?>
                            <tr data-transaction-id="<?= $transaksi->id_transaksi ?>" data-created-at="<?= $transaksi->waktu ?>">
                                <td class="text-center"><?= $no++ ?></td>
                                <td><?= date('d M Y H:i:s', strtotime($transaksi->waktu)) ?></td>

                                <td class="text-end editable-cell" data-field="setor_sw" data-created="<?= $transaksi->waktu ?>">
                                    <?= $transaksi->setor_sw > 0 ? 'Rp ' . number_format($transaksi->setor_sw, 0, ',', '.') : '-' ?>
                                </td>
                                <td class="text-end editable-cell" data-field="tarik_sw" data-created="<?= $transaksi->waktu ?>">
                                    <?= $transaksi->tarik_sw > 0 ? 'Rp ' . number_format($transaksi->tarik_sw, 0, ',', '.') : '-' ?>
                                </td>

                                <td class="text-end editable-cell" data-field="setor_swp" data-created="<?= $transaksi->waktu ?>">
                                    <?= $transaksi->setor_swp > 0 ? 'Rp ' . number_format($transaksi->setor_swp, 0, ',', '.') : '-' ?>
                                </td>
                                <td class="text-end editable-cell" data-field="tarik_swp" data-created="<?= $transaksi->waktu ?>">
                                    <?= $transaksi->tarik_swp > 0 ? 'Rp ' . number_format($transaksi->tarik_swp, 0, ',', '.') : '-' ?>
                                </td>

                                <td class="text-end editable-cell" data-field="setor_ss" data-created="<?= $transaksi->waktu ?>">
                                    <?= $transaksi->setor_ss > 0 ? 'Rp ' . number_format($transaksi->setor_ss, 0, ',', '.') : '-' ?>
                                </td>
                                <td class="text-end editable-cell" data-field="tarik_ss" data-created="<?= $transaksi->waktu ?>">
                                    <?= $transaksi->tarik_ss > 0 ? 'Rp ' . number_format($transaksi->tarik_ss, 0, ',', '.') : '-' ?>
                                </td>

                                <td class="text-end editable-cell" data-field="setor_sp" data-created="<?= $transaksi->waktu ?>">
                                    <?= $transaksi->setor_sp > 0 ? 'Rp ' . number_format($transaksi->setor_sp, 0, ',', '.') : '-' ?>
                                </td>
                                <td class="text-end editable-cell" data-field="tarik_sp" data-created="<?= $transaksi->waktu ?>">
